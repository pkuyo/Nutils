...
namespace Nutils.MixedUI
{
    public class OpKeyBinderEx : UIconfig
    {
        ...
        public static bool GetKey(string action)
        {
            if (action.Contains("Axis"))
            {
                ...
                if (ReInput.controllers?.Joysticks == null)
                    return false;
                if (colIndex <= '9' && colIndex > '0' && ReInput.controllers.Joysticks.Count > (colIndex - '0' - 1))
                    return ReInput.controllers.Joysticks[colIndex - '0' - 1].GetAxisTimeActiveById(action[1] - '0') != 0;
                else
                {
                    foreach (var col in ReInput.controllers.Joysticks)
                    {
                        if (col.GetAxisTimeActiveById(action[0] - '0') != 0)
                            return true;
                    }
                }
                return false;
        ...
        public static bool GetKeyDown(string action)
        {
            if (action.Contains("Axis"))
            {
                action = action.Replace("JoystickController", "");
                var colIndex = action[0];
                action = action.Replace(" Axis ", "");
                if (ReInput.controllers?.Joysticks == null)
                    return false;
                if (colIndex <= '9' && colIndex > '0' && ReInput.controllers.Joysticks.Count > (colIndex - '0' - 1))
                    return ReInput.controllers.Joysticks[action[0] - '0' - 1].GetAxisTimeActiveById(action[1] - '0') != 0 &&
                           ReInput.controllers.Joysticks[action[0] - '0' - 1].GetAxisLastTimeActiveById(action[1] - '0') == 0;
                else
                {
                    foreach (var col in ReInput.controllers.Joysticks)
                    {
                        if (col.GetAxisTimeActiveById(action[0] - '0') != 0 && col.GetAxisLastTimeActiveById(action[0] - '0') == 0)
                            return true;
                    }
                }
                return false;
